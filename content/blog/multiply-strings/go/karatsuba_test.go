...
func Test_multiply(t *testing.T) {
	...
	for _, testCase := range cases {
		t.Run(testCase.name, func(t *testing.T) {
			res := multiplyOptimised(testCase.num1, testCase.num2)
			assert.Equal(t, testCase.expected, res)
		})
	}
}

func Test_sumStrings(t *testing.T) {
	cases := []struct {
		name     string
		num1     string
		num2     string
	...
	for _, testCase := range cases {
		t.Run(testCase.name, func(t *testing.T) {
			res := sumStrings(testCase.num1, testCase.num2)
			assert.Equal(t, testCase.expected, res)
		})
	}
}
...
// Benchmark_multiply-10    	    1006	   1032970 ns/op
...
// Benchmark_multiplyRecursion-10    	    1548	    763014 ns/op
// Benchmark_multiply-10    	    1006	   1032970 ns/op
// Benchmark_multiplyOptimised-10    	    2874	    375412 ns/op
func Benchmark_multiplyOptimised(b *testing.B) {
	for i := 0; i < b.N; i++ {
		multiplyOptimised(
			"3141592653589793238462643383279502884197169399375105820974944592",
			"2718281828459045235360287471352662497757247093699959574966967627",
